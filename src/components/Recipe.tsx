    ...
    return (
        <TouchableOpacity className="flex-1 h-48 rounded-[20px] overflow-hidden" activeOpacity={0.8} {...rest}>
            <ImageBackground source={{ uri: recipe.image }} className="flex-1">
                <LinearGradient colors={['rgba(0,0,0,0.5)', '#000']} className="flex-1 p-3 justify-end">
                    <Text className="text-white text-sm font-bold" numberOfLines={1} lineBreakMode="tail">
                        {recipe.name}
                    </Text>

                    <Text className="text-yellow-500 font-regular text-xs" numberOfLines={1} lineBreakMode="tail">
